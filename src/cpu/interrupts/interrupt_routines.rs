...
pub(super) extern "x86-interrupt" fn non_maskable_interrupt_handler(
...
pub(super) extern "x86-interrupt" fn bound_range_exceeded_handler(
    stack_frame: InterruptStackFrame,
) {
...
pub(super) extern "x86-interrupt" fn coprocessor_segment_overrun_handler(
    stack_frame: InterruptStackFrame,
) {
    crate::println!(
        "COPROCESSOR SEGMENT OVERRUN INTERRUPT stack_frame: {:#?}",
        stack_frame
    );
}
...
pub(super) extern "x86-interrupt" fn x87_floating_point_exception_handler(
    stack_frame: InterruptStackFrame,
) {
    crate::println!(
...
pub(super) extern "x86-interrupt" fn alignement_check_handler(
    stack_frame: InterruptStackFrame,
    error: usize,
) {
    crate::println!(
...
pub(super) extern "x86-interrupt" fn machine_check_handler(stack_frame: InterruptStackFrame) {
    crate::println!("MACHINE CHECK INTERRUPT stack_frame: {:#?}", stack_frame,);
}
...
pub(super) extern "x86-interrupt" fn virtualization_exception_handler(
    stack_frame: InterruptStackFrame,
) {
    crate::println!(
...
pub(super) extern "x86-interrupt" fn control_protection_exception_handler(
    stack_frame: InterruptStackFrame,
    error: usize,
) {
    crate::println!(
